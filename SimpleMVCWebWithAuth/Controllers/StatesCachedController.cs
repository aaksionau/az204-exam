...
{
    //TODO: (chapter 9) Only use this controller when working on Caching data
    //       remove the auth if you skipped chapter 7
    [Authorize(Roles = "Admin")]
    public class StatesCachedController : Controller
    {
        ...
        public StatesCachedController(ApplicationDbContext context, IConfiguration configuration, Task<RedisConnection> redisConnectionFactory)
        {
            _context = context;
        ...
        private async Task<List<State>> AddOrUpdateStatesInCache()
        {
            ...
            await _redisConnection.BasicRetryAsync(async (db) =>
                            await db.StringSetAsync(STATES_KEY, statesJSON));
        ...
        {
            _redisConnection = await _redisConnectionFactory;
            var result = (await _redisConnection.BasicRetryAsync(async (db) =>
                            await db.StringGetAsync(STATES_KEY))).ToString();
